...
def config(mesh, manifold, element_layout, enm=None):
    """"""
    ...
    mesh._parse_elements_from_element_layout(element_layout, enm=enm)
...
class MsePyMesh(Frozen):
    ...
    def _parse_elements_from_element_layout(self, element_layout, enm=None):
        """"""
                ...
                if isinstance(layout_ij, (int, float)):
                    ...
                    for _ in layout_ij:
                        assert isinstance(_, (int, float)) and _ > 0, \
                            f"element_layout of region #{i} = {layout_i} is illegal."
                    layout_ij = np.array(layout_ij)
                    layout_ij /= np.sum(layout_ij)

                assert np.sum(layout_ij) == 1, f"scale layout array into 1 pls."
                _temp.append(layout_ij)

            layout[i] = _temp

        self.elements._generate_elements_from_layout(layout, enm=enm)
