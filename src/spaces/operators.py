...
def wedge(s1, s2):
    ...
    if s1.__class__.__name__ == 'ScalarValuedFormSpace' and s2.__class__.__name__ == 'ScalarValuedFormSpace':
        ...
        return new('Omega', k + l_, mesh=s1.mesh, orientation=orientation)
...
def Hodge(space):
    ...
    if space.__class__.__name__ == 'ScalarValuedFormSpace':
        n = space.mesh.ndim
        return new('Omega', n - space.k, mesh=space.mesh, orientation=space.orientation)
    else:
        raise NotImplementedError()
...
def d(space):
    ...
    if space.__class__.__name__ == 'ScalarValuedFormSpace':
        ...
        return new('Omega', space.k + 1, mesh=space.mesh, orientation=space.orientation)
    else:
        raise NotImplementedError()
...
def trace(space):
    if space.__class__.__name__ == 'ScalarValuedFormSpace':
        ...
        return new('Omega', space.k, mesh=boundary_mesh, orientation=space.orientation)
